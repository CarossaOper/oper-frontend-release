<template>
  <header
    class="block fixed w-full mt-0 mb-0 bg-white justify-between px-4 md:px-12 transition-all duration-200">
    <div class="flex md:block transition-all duration-200" :class="{'md:flex': scrolled}">
      <div class="inline-block align-middle transition-all duration-200"
        :class="{'inline-block': scrolled, 'md:block': !scrolled}">
        <nuxt-link class="block" to="/">
          <img class="align-middle my-6 h-16" src="../static/logo_bw_opera.png" alt="CarossaOper Logo">
        </nuxt-link>
      </div>
      <div class="inline-block mx-auto absolute md:static right-[5%] self-center transition-all duration-200"
        :class="{'md:flex': scrolled || !collapsed, 
        'float-right': scrolled, 'self-center': scrolled, 'mr-0': scrolled,
        'md:block': !scrolled}">
        <nav class="w-full">
          ...
          <ul class="fixed block justify-center font-vollkornsc bg-white text-black text-sm xl:text-lg
                      left-0 right-0
                      mt-10
                      min-h-screen px-4 space-y-4
                    
                      transform transition duration-300

                      md:relative md:flex md:min-h-0 md:px-0 md:mt-0 md:space-y-0 md:translate-x-0 
                      " :class="{'translate-x-full': collapsed, 'translate-x-0': !collapsed, 'text-green-600': !scrolled}">
            <li class="p-4 md:mx-auto" v-on:click="extend"><a href="#termine">Termine &amp; Karten</a></li>
            <li class="p-4 md:mx-auto"><a href="https://blog.carossa-oper.de/">Neuigkeiten</a></li>
            <li class="p-4 md:mx-auto"><NuxtLink to="/about">Über Uns</NuxtLink></li>
            <li class="p-4 md:mx-auto" v-on:click="extend"><a>Unsere Unterstützer</a></li>
            <li class="p-4 md:mx-auto"><a href="https://www.carossa-gymnasium.de/">Die Schule</a></li>
          </ul>
        </nav>
      </div>
    </div>
